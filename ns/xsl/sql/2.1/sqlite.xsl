...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:sql="http://xsd.nore.fr/sql/2.1">
	...
	<xsl:template match="sql:table/sql:primarykey">
		<xsl:choose>
			<xsl:when test="../sql:column/sql:datatype/sql:integer/@autoincrement = 'yes'">
	...
	<xsl:template match="sql:table/sql:column">
		...
		<xsl:variable name="isAutoIncrement" select="(./sql:datatype/sql:integer/@autoincrement = 'yes')" />
