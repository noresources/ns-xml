...
<xsl:stylesheet version="1.0"
	...
	<!-- Subcommand result struct name -->
	<xsl:template
		name="prg.c.parser.subcommandResultStructName">
		...
		<xsl:call-template name="prg.c.parser.structName">
			<xsl:with-param name="name">
				...
				<xsl:call-template name="c.validIdentifierName">
					<xsl:with-param name="name"
						select="$subcommandNode/prg:name" />
				</xsl:call-template>
	...
	<xsl:template name="prg.c.parser.optionResultDeclaration">
		...
		<xsl:call-template
			name="c.structVariableDeclaration">
			<xsl:with-param name="name" select="$structName" />
			<xsl:with-param name="variableName">
				<xsl:apply-templates
					select="$optionNode/prg:databinding/prg:variable" />
			</xsl:with-param>
			<xsl:with-param name="variableNameStyle"
				select="'none'" />
			<!-- TEMP -->
	...
	<xsl:template name="prg.c.parser.optionResultsDeclaration">
		...
		<xsl:variable name="rootOptionsNode"
			select="$rootNode/prg:options" />
		<xsl:for-each
			select="$rootOptionsNode//*[prg:databinding/prg:variable]">
			<xsl:call-template
				name="prg.c.parser.optionResultDeclaration" />
			<xsl:value-of select="$str.endl" />
		</xsl:for-each>
	</xsl:template>

	<xsl:variable
		name="prg.c.parser.subcommandResultStructBaseName"
		select="'subcommand_result'" />

	<xsl:template
		name="prg.c.parser.subcommandResultDefinition">
		<xsl:param name="subcommandNode" select="." />
		<xsl:param name="structName">
			<xsl:call-template
				name="prg.c.parser.subcommandResultStructName">
				<xsl:with-param name="subcommandNode"
					select="$subcommandNode" />
			</xsl:call-template>
		</xsl:param>
		<xsl:call-template name="c.structDefinition">
			...
			<xsl:with-param name="content">
				<xsl:call-template name="c.inlineComment">
					<xsl:with-param name="content"
						select="'Subcommand options'" />
				</xsl:call-template>
				<xsl:value-of select="$str.endl" />
				<xsl:call-template
					name="prg.c.parser.optionResultsDeclaration">
	...
	<xsl:template name="prg.c.parser.programResultDefinition">
		...
		<xsl:variable name="structName">
			<xsl:value-of
				select="$prg.c.parser.structName.program_result" />
		</xsl:variable>

		<!-- Program subcommand result structs -->
		<xsl:if
			test="$programNode/prg:subcommands[prg:subcommand/prg:options]">
			<xsl:value-of select="$str.endl" />

			<xsl:for-each
				select="$programNode/prg:subcommands/prg:subcommand[prg:options]">
				<xsl:variable name="scBaseName">
					<xsl:call-template
						name="prg.c.parser.subcommandResultStructName" />
				</xsl:variable>
				<xsl:call-template
					...
					<xsl:with-param name="structName">
						<xsl:text>_</xsl:text>
						<xsl:value-of select="$scBaseName" />
					</xsl:with-param>
				</xsl:call-template>
		...
		<!-- End of Program subcommand result structs -->


		<!-- Main program result structure -->
		<xsl:call-template name="c.structDefinition">
			<xsl:with-param name="name"
				select="concat('_', $structName)" />
			<xsl:with-param name="content">
				<!-- Base -->
				<!-- Parser messages -->
				<xsl:call-template name="c.inlineComment">
					<xsl:with-param name="content"
						select="'Messages - Sorted by severity'" />
				</xsl:call-template>
				<xsl:value-of select="$str.endl" />
				<xsl:value-of
					select="$prg.c.parser.structName.nsxml_message" />
				<xsl:text> *messages[</xsl:text>
				<xsl:value-of
					select="$prg.c.parser.variableName.nsxml_message_type_count" />
				<xsl:text>];</xsl:text>
				<xsl:value-of select="$str.endl" />
				<xsl:call-template name="c.inlineComment">
					<xsl:with-param name="content"
						select="'Messages - Sorted by apparition'" />
				</xsl:call-template>
				<xsl:value-of select="$str.endl" />
				<xsl:value-of
					select="$prg.c.parser.structName.nsxml_message" />
				...
				<xsl:call-template name="c.variableDeclaration">
					<xsl:with-param name="type"
						select="$prg.c.parser.structName.nsxml_value" />
					<xsl:with-param name="nameStyle" select="'none'" />
					<xsl:with-param name="name" select="'values'" />
					<xsl:with-param name="pointer" select="1" />
				</xsl:call-template>
				<xsl:value-of select="$str.endl" />
				<!-- End of static part (nsxml_program_result) -->
	...
	<xsl:template
		name="prg.c.parser.programResultFreeFunctionDeclaration">
		<xsl:param name="programNode" select="." />
		<xsl:call-template name="c.functionDeclaration">
			<xsl:with-param name="name"
				select="$prg.c.parser.functionName.program_result_free" />
	...
	<xsl:template
		name="prg.c.parser.programResultFreeFunctionDefinition">
		<xsl:param name="programNode" select="." />
		<xsl:call-template name="c.functionDefinition">
			...
			<xsl:with-param name="content">
				...
				<xsl:value-of select="$str.endl" />
				<xsl:call-template
					name="prg.c.parser.option_resultFunctionCall">
					<xsl:with-param name="programNode"
						select="$programNode" />
					<xsl:with-param name="functionName"
						select="'nsxml_option_result_cleanup'" />
	...
	<xsl:template
		name="prg.c.parser.programResultErrorCountFunctionDeclaration">
		<xsl:param name="programNode" select="." />
		<xsl:call-template name="c.functionDeclaration">
			<xsl:with-param name="name"
				select="$prg.c.parser.functionName.program_result_error_count" />
	...
	<xsl:template
		name="prg.c.parser.programResultErrorCountFunctionDefinition">
		<xsl:param name="programNode" select="." />
		<xsl:call-template name="c.functionDefinition">
			<xsl:with-param name="name"
				select="$prg.c.parser.functionName.program_result_error_count" />
			...
			<xsl:with-param name="content">
				...
				<xsl:value-of
					select="$prg.c.parser.variableName.nsxml_message_type_error" />
				<xsl:text>]) + nsxml_message_count(result-&gt;messages[</xsl:text>
				<xsl:value-of
					select="$prg.c.parser.variableName.nsxml_message_type_fatal_error" />
				<xsl:text>]));</xsl:text>
			</xsl:with-param>
		</xsl:call-template>
	</xsl:template>
	...
	<xsl:template
		name="prg.c.parser.programResultGetMessageFunctionName">
		<xsl:param name="type" />
			...
			<xsl:with-param name="name">
				<xsl:choose>
					<xsl:when test="$type = 'fatal_error'">
						<xsl:value-of
							select="concat(concat($prg.c.parser.prefix, '_result_get_'), $type)" />
					</xsl:when>
					<xsl:otherwise>
						<xsl:value-of
							select="concat(concat(concat($prg.c.parser.prefix, '_result_get_'), $type), 's')" />
					</xsl:otherwise>
				</xsl:choose>
			</xsl:with-param>
		</xsl:call-template>
	...
	<xsl:template
		name="prg.c.parser.programResultGetMessageFunctionDeclaration">
		<xsl:param name="type" />
		<xsl:call-template name="c.functionDeclaration">
			<xsl:with-param name="name">
				<xsl:call-template
			...
			<xsl:with-param name="returnType"
				select="concat($prg.c.parser.structName.nsxml_message, ' *')" />
			<xsl:with-param name="parameters">
				<xsl:value-of
					select="$prg.c.parser.structName.program_result" />
				<xsl:text> *result</xsl:text>
	...
	<xsl:template
		name="prg.c.parser.programResultGetMessageFunctionDefinition">
		<xsl:param name="type" />
		<xsl:call-template name="c.functionDefinition">
			<xsl:with-param name="name">
				<xsl:call-template
			...
			<xsl:with-param name="content">
				...
				<xsl:choose>
					<xsl:when test="$type = 'warning'">
						<xsl:value-of
							select="$prg.c.parser.variableName.nsxml_message_type_warning" />
					</xsl:when>
	...
	<xsl:variable
		name="prg.c.parser.programResultDisplayErrorsFunctionSignature">
		<xsl:call-template name="c.functionSignature">
			<xsl:with-param name="returnType" select="'void'" />
			<xsl:with-param name="name">
				<xsl:value-of
					select="$prg.c.parser.functionName.program_result_display_errors" />
			</xsl:with-param>
			<xsl:with-param name="parameters">
	...
	<xsl:template
		name="prg.c.parser.programResultDisplayErrorsFunctionDeclaration">
		<xsl:call-template name="c.functionDeclaration">
			<xsl:with-param name="signature"
				select="$prg.c.parser.programResultDisplayErrorsFunctionSignature" />
		</xsl:call-template>
	</xsl:template>

	<xsl:template
		name="prg.c.parser.programResultDisplayErrorsFunctionDefinition">
		<xsl:call-template name="c.functionDefinition">
			<xsl:with-param name="signature"
				select="$prg.c.parser.programResultDisplayErrorsFunctionSignature" />
			<xsl:with-param name="content">
