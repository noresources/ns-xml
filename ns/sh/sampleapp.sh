...
Usage: 
  ...
  With global options:
    ...
    Exclusive option group
      ...
      Nested exclusive group
        --numeric-argument: Numeric argument
          Only numberrs are accepted as argument value. A numeric argument 
          appears as a spin box in the UI
        --float-argument: Float argument
          Numeric argument with decimals. A minimum (1.0) and maximum (10) 
    ...
    -H, --hostname: Hostname
      Accept a host name. In console mode, the autocompletion will propose 
    ...
    -e, --non-strict-enum: Non-restrictive enumeration
      Non restricive enumeration will only propose some values in 
      autocompletion but will accept any other values  
      The argument can be:  
        nOptionA, nValueB, nItemC or nItemD with space
  This application demonstrates most of the possibilities offered by the 
  program interface definition schema to define and display command line 
  options
EOFUSAGE
}
