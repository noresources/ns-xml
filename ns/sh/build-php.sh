#!/usr/bin/env bash
# ####################################
# Copyright © 2018 - 2020 by Renaud Guillard
...
parse_setoptionpresence()
{
	...
	G_2_g_1_g_4_merge)
		...
		;;
	G_2_g_2_parser_namespace)
		;;
	G_2_g_3_program_namespace)
		;;
	G_2_g_4_classname)
		;;
	G_3_g_1_output)
...
parse_process_option()
{
	...
	then
		...
		parser-namespace | parser-ns)
			...
			parser_optionhastail=false
			[ "${parser_item:0:2}" = "\-" ] && parser_item="${parser_item:1}"
			! parse_setoptionpresence G_2_g_2_parser_namespace && return ${PARSER_ERROR}
		...
		program-namespace | program-ns | prg-ns)
			...
			parser_optionhastail=false
			[ "${parser_item:0:2}" = "\-" ] && parser_item="${parser_item:1}"
			! parse_setoptionpresence G_2_g_3_program_namespace && return ${PARSER_ERROR}
		...
		classname)
			...
			parser_optionhastail=false
			[ "${parser_item:0:2}" = "\-" ] && parser_item="${parser_item:1}"
			! parse_setoptionpresence G_2_g_4_classname && return ${PARSER_ERROR}
