...
__xsltdeps_bashcompletion()
{
	...
	local globalargs="--absolute --relative --add-input --help --debug -i -d"
	...
	local option="$(__xsltdeps_getoptionname ${previous})"
	if [ ! -z "${option}" ]
	then
		case "${option}" in
		"relative" | "r")
			__xsltdeps_appendfsitems "${current}"  -type d 
...
complete -o nospace -F __xsltdeps_bashcompletion xsltdeps.sh
