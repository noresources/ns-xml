...
__build_php_getfindpermoptions()
{
	local access="${1}"
	local res=""
	while [ ! -z "${access}" ]
...
__build_php_bashcompletion()
{
	...
	local globalargs="--xml-description --skip-validation --no-validation --base --info --embed --merge --parser-namespace --parser-ns --program-namespace --program-ns --prg-ns --classname --output --ns-xml-path --ns-xml-path-relative --help"
	...
	local option="$(__build_php_getoptionname ${previous})"
	if [ ! -z "${option}" ]
	then
		case "${option}" in
		"xml-description" | "x")
...
complete -o nospace -F __build_php_bashcompletion build-php.sh
